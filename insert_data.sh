...
do
  ...
  then
    ...
    #insert into games
    INSERT_GAMES_RESULT=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES($YEAR, '$ROUND', $WINNER_ID, $OPPO_ID, $WIN_GOALS, $OPPO_GOALS);")
    if [[ $INSERT_GAMES_RESULT == 'INSERT 0 1' ]]
      then
        echo "Inserted into game, $YEAR : $ROUND $WINNER ($WIN_GOALS) - ($OPPO_GOALS) $OPPO"
    fi
  fi
done